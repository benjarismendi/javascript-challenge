...
export const listPaddockManagerIds = () => {
  const managerIds = paddockManagers
...
export const sortPaddockTypeByTotalArea = () => {
  let paddocksAreas = paddockType
    .map((type) => {
      ...
      return { name: type.name, area };
    })
    .sort((a, b) => b.area - a.area);
  return paddocksAreas.map((type) => type.name);
};
...
export const sortFarmManagerByAdminArea = () => {
  let farmersManagedAreas = paddockManagers.map(manager => {
    let totalArea = 0;
    paddocks.map(paddock => {
      if(paddock.paddockManagerId === manager.id) totalArea += paddock.area
    });
    return {name: manager.name, totalArea}
  }).sort((a, b) => b.totalArea - a.totalArea);
  return farmersManagedAreas.map(farmer => farmer.name)
};
